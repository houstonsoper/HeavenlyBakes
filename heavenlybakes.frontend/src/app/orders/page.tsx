...
export default function orders(){
    ...
    useEffect(() => {
        ...
        const getOrders  = async () => {
            if (auth.user != null) {
                const fetchedOrders : orderWithOrderItems[] = await getOrdersByUserId({userId : auth.user.userId, limit, offset: (page - 1) * limit}, signal);
    ...
    useEffect(() => {
        ...
        const getOrdersForNextPage = async () => {
            if (auth.user != null) {
                const fetchedOrders : orderWithOrderItems []  = await getOrdersByUserId({userId : auth.user.userId, limit, offset: page * limit}, signal);
